...
async def on_media_sent(
        ...
        widget: Any,
        dialog_manager: DialogManager,
        config: FromDishka[Config]  # Получаем Config через DI
):
    """
    ...
    try:
        if message.photo:
            ...
            media_url = await save_media(await media_file.read(), video.mime_type, config.media)
...
async def on_publish_now(
    ...
    post = PostRequestDTO(
        name=post_data["name"],
        text=post_data["text"],
        image_link=post_data.get("media_path"),
        is_publish_now=True,
        publish_date=None,
        is_checked=False,
        sender_id=user.id
...
async def on_time_entered(
    ...
    post = PostRequestDTO(
        name=post_data["name"],
        text=post_data["text"],
        image_link=post_data.get("media_url"),
