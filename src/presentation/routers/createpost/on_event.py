...
async def on_publish_now(
    ...
    # Check if user can publish now
    can_publish, reason = await post_service.can_user_publish_now(user.id)
    if not can_publish:
        dialog_manager.dialog_data["limit_reason"] = reason
        await dialog_manager.switch_to(PostSG.publish_limit_warning)
        return
    ...
    post = PostRequestDTO(
        ...
        is_paid=is_paid,
        sender_id=user.id,
        created_at=datetime.now()  # Исправлено
    )

    try:
        created_post = await post_service.add_post(post)
        await callback.message.answer("✅ Пост отправлен на модерацию! Он будет опубликован сразу после одобрения.")
    except Exception as e:
        await callback.message.answer(f"❌ Ошибка при отправке на модерацию: {str(e)}")
...
async def on_time_entered(
    ...
    # Check if user can schedule at this time
    can_schedule, reason = await post_service.can_schedule_post(user.id, scheduled_datetime)
    if not can_schedule:
